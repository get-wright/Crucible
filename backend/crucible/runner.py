...
async def execute_run(
    ...
    *,
    ...
    judge_model, judge_note = resolve_model(ir.scenario.judge_model or "", s.judge_model)
    ...
    try:
        ...
        if limit_hit is not None:
            log.emit("limit.hit", {"limit": limit_hit[0], "value": limit_hit[1]})
        ...
        if judge:
            ...
            judge_items, judge_usage = await judge_run(
                ir, log, checks, client=c, settings=s
            )
            for item in judge_items:
                log.emit("judge.item", item.as_dict())
        ...
        usage = Usage(totals["input_tokens"], totals["output_tokens"]) + seed_usage + judge_usage + users.usage
        return RunResult(
            run_id=rid,
            scenario_hash=ir.scenario_hash,
            variant=variant,
